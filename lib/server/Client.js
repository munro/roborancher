...
function Client(socket) {
    ...
    socket.on('registers_done', function (registers) {
        ...
        if (that.room && that.hand) {
            that.registers = [];
            for (var i = 0; i < 5; i += 1) {
                if (registers && typeof registers[i] !== 'undefined' &&
                        registers[i] >= 0 && registers[i] < that.hand.length) {
                    that.registers.push(that.hand[registers[i]]);
                    that.hand[registers[i]] = null;
                }
            }
            that.registers = that.registers.filter(function (v) {
                return v !== null;
            });
            that.hand = that.hand.filter(function (v) {
                return v !== null;
            });
            for (var i = registers.length; i < 5; i += 1) {
                that.registers.push(that.hand.pop());
            }
            that.hand = false;
            console.log('REGISTERS SET', that.registers);
            that.room.startRegisterTimer();
        }
    });
