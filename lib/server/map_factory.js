...
exports.canMoveForward = function(bot) {
...
exports.canMoveBackward = function(bot) {
    var tile = getTileAtPosition(bot.position);
    var collisions = DIRECTION_COLLISION_MAP[bot.direction.name];
    if (tile.index == collisions[1].index)
        return false;
...
function moveBot(bot, direction, reps) {
    var oldDirection = bot.direction;
    bot.direction = direction;
    for (var i = 0; i < reps; i++) {
        if (!exports.canMoveForward(bot))
            break;
        bot.forwards();
    }
...
exports.execTileMotion = function(bot) {
    var tile = getTileAtPosition(bot.position);
    ...
    switch(motion[0]) {
        case 'u':
            moveBot(bot, dir.north, reps);
            break;
        case 'd':
            moveBot(bot, dir.south, reps);
            break;
        case 'r':
            moveBot(bot, dir.east, reps);
            break;
        case 'l':
            moveBot(bot, dir.west, reps);
            break;            
        case 'c':
            for (var i = 0; i < reps; i++)
                bot.rotate_cw();
