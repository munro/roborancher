...
require({
    ...
    socket.on('chat_message_from_server', function(player, msg) {
        var playerHtml = $('<span class="name">').html(player);
        var msgHtml = $('<span class="message">').html(msg);
        html = '<div><span class="pre">&lt;</span><span class="name">' + player + '</span/>'
                + '<span class="post">&gt;</span> <span class="message">' + msg + '</span></div>';
        $('#game .chat .messages').append(html);
        $('#game .chat .messages').scrollTop($('#game .chat .messages')[0].scrollHeight);
    });
    
    socket.on('start_game', function (map) {
