...
$(function () {
    ...
    socket.on('start_round', function (seconds) {
        ...
        function setStatus(seconds) {
            $('#game .hand .message').html('Get ready to set your registers!  In ' +
        ...
        start_timer = setInterval(function () {
            ...
            if (seconds > 0) {
                ...
                clearInterval(start_timer);
                $('#game .hand .message').html('Get ready to set your registers!');
            }
        }, 1000);
    });

    socket.on('deal_cards', function (hand) {
        $('#game .hand .message').html('Place your registers in the order you want your robot to move.');
        clearInterval(start_timer);
        $('#my_hand li').remove();
        $('#my_hand').show();
        $('#registers_done').show();
    ...
    $('#registers_done').click(function () {
        socket.emit('registers_done', $('#my_registers .card').map(function () {
    ...
    socket.on('deal_timer', function (seconds) {
        function setStatus(seconds) {
            $('#game .hand .message').html('Someone has locked in!  You have ' +
                seconds + ' second' + (seconds === 1 ? '' : 's') + ' to set your registers.');
        }
    });
