...
State.prototype.exec = function (args) {
    this.emit.apply(this.graph, ['before'].concat(args));
    this.fn.apply(this.graph, args);
    this.emit.apply(this.graph, ['after'].concat(args));
};
...
StateGraph.prototype.state = function (name, fn) {
    this.states[name] = new State(this, fn);
}

StateGraph.prototype.go = function (name) {
    var args = Array.prototype.slice.call(arguments, 1);
    this.states[name] && this.states[name].exec(args);
}
