...
function updatePlayers(players) {
    ...
    players.forEach(function(player) {
        ...
        var image2 = '/assets/bots/robot-down-' + PLAYER_TO_COLOR[player.player] + '.png';
        var tr = $('<tr />')
            .append($('<td class="icon" />')
                .append($('<img />').attr('src', image2))
            )
            .append($('<td class="name" />').text(player.name))
            .append($('<td class="icon" />').text(player.flags));
...
$(function () {
    ...
    socket.on('update_flags', function (flags) {
        ...
        console.log('flaggggss', html);
        $('#board .flags').html(html);
    });
        ...
        function popStack() {
            ...
            if (!instr) {
                ...
                if (game_over) {
                    $('#my_hand').hide();
                    $('.registers').hide();
                    $('#board .hand .message').text('Game over!');
                    $('.hand .message').text('Game over!  Check the player list for final scores.');
                    $('.hand').css('background-color', '#9AF08F');
                    // Place your registers in the order you want your robot to move.
                } else {
                    $('#my_registers .card').remove();
                }
                return;
        ...
        room.clients.forEach(function (client) {
            $('#lobby .players').append(
                $('<tr />')
                    ...
                    .append($('<td class="player" />').text('Player ' + client.player))
            );
        });
    };
